...
public class Request4CommentCounts extends Request<ArrayList<CommentNumber>> {
	private Response.Listener<ArrayList<CommentNumber>> mListener;

	public Request4CommentCounts(String url, Response.Listener<ArrayList<CommentNumber>> listener, Response
		...
		try {
			...
			for (String comment_ID : comment_IDs) {
				if (!jsonObject.isNull(comment_ID)) {
					...
					commentNumber.setThread_id(jsonObject.getJSONObject(comment_ID).getString(CommentNumber
							.THREAD_ID));
					commentNumber.setThread_key(jsonObject.getJSONObject(comment_ID).getString(CommentNumber
							.THREAD_KEY));
