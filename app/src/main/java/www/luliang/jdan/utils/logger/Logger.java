package www.luliang.jdan.utils.logger;
	...
	private static void logHeaderContent(int logType, String tag, int methodCount) {
		...
		for (int i = methodCount; i > 0; i--) {
			...
			builder.append("║ ").append(level).append(getSimpleClassName(trace[stackIndex].getClassName())).append(""
					+ ".").append(trace[stackIndex].getMethodName()).append(" ").append(" (").append(trace[stackIndex]
					.getFileName()).append(":").append(trace[stackIndex].getLineNumber()).append(")");
			level += "   ";
